/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import * as nestAccessControl from "nest-access-control";
import * as defaultAuthGuard from "../../auth/defaultAuth.guard";
import { DbQueryConnectionService } from "../dbQueryConnection.service";
import { AclValidateRequestInterceptor } from "../../interceptors/aclValidateRequest.interceptor";
import { AclFilterResponseInterceptor } from "../../interceptors/aclFilterResponse.interceptor";
import { DbQueryConnectionCreateInput } from "./DbQueryConnectionCreateInput";
import { DbQueryConnection } from "./DbQueryConnection";
import { DbQueryConnectionFindManyArgs } from "./DbQueryConnectionFindManyArgs";
import { DbQueryConnectionWhereUniqueInput } from "./DbQueryConnectionWhereUniqueInput";
import { DbQueryConnectionUpdateInput } from "./DbQueryConnectionUpdateInput";

@swagger.ApiBearerAuth()
@common.UseGuards(defaultAuthGuard.DefaultAuthGuard, nestAccessControl.ACGuard)
export class DbQueryConnectionControllerBase {
  constructor(
    protected readonly service: DbQueryConnectionService,
    protected readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}
  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Post()
  @swagger.ApiCreatedResponse({ type: DbQueryConnection })
  @nestAccessControl.UseRoles({
    resource: "DbQueryConnection",
    action: "create",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async createDbQueryConnection(
    @common.Body() data: DbQueryConnectionCreateInput
  ): Promise<DbQueryConnection> {
    return await this.service.createDbQueryConnection({
      data: data,
      select: {
        createdAt: true,
        dbConnectionString: true,
        dbProvider: true,
        id: true,
        queryString: true,
        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get()
  @swagger.ApiOkResponse({ type: [DbQueryConnection] })
  @ApiNestedQuery(DbQueryConnectionFindManyArgs)
  @nestAccessControl.UseRoles({
    resource: "DbQueryConnection",
    action: "read",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async dbQueryConnections(
    @common.Req() request: Request
  ): Promise<DbQueryConnection[]> {
    const args = plainToClass(DbQueryConnectionFindManyArgs, request.query);
    return this.service.dbQueryConnections({
      ...args,
      select: {
        createdAt: true,
        dbConnectionString: true,
        dbProvider: true,
        id: true,
        queryString: true,
        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: DbQueryConnection })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "DbQueryConnection",
    action: "read",
    possession: "own",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async dbQueryConnection(
    @common.Param() params: DbQueryConnectionWhereUniqueInput
  ): Promise<DbQueryConnection | null> {
    const result = await this.service.dbQueryConnection({
      where: params,
      select: {
        createdAt: true,
        dbConnectionString: true,
        dbProvider: true,
        id: true,
        queryString: true,
        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: DbQueryConnection })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "DbQueryConnection",
    action: "update",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async updateDbQueryConnection(
    @common.Param() params: DbQueryConnectionWhereUniqueInput,
    @common.Body() data: DbQueryConnectionUpdateInput
  ): Promise<DbQueryConnection | null> {
    try {
      return await this.service.updateDbQueryConnection({
        where: params,
        data: data,
        select: {
          createdAt: true,
          dbConnectionString: true,
          dbProvider: true,
          id: true,
          queryString: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: DbQueryConnection })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "DbQueryConnection",
    action: "delete",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async deleteDbQueryConnection(
    @common.Param() params: DbQueryConnectionWhereUniqueInput
  ): Promise<DbQueryConnection | null> {
    try {
      return await this.service.deleteDbQueryConnection({
        where: params,
        select: {
          createdAt: true,
          dbConnectionString: true,
          dbProvider: true,
          id: true,
          queryString: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
